...
Model-facing Consumer for `ctx.notebooks`. Registers twelve tools — `notebook_open`, `notebook_create`, `notebook_read`, `notebook_edit_cell`, `notebook_insert_cell`, `notebook_delete_cell`, `notebook_move_cell`, `notebook_copy_cell`, `notebook_execute`, `notebook_restart`, `notebook_reload`, and `notebook_inspect` — plus `executeNotebookCellAsUser()` for human-initiated execution paths.
