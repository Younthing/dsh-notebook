...
function validate(events: readonly SessionEvent[], seedLength: number, fail: InvariantFailure): void {
  try {
    foldNotebooks(events, seedLength)
  } catch (error: unknown) {
    /* v8 ignore next -- foldNotebooks normalizes every rejected stream to NotebookLogError. */
    if (!(error instanceof NotebookLogError)) throw error
    fail(error.message)
  }
}
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ...
  ctx.on('internal/dispatch', (_mode, eventName, args) => {
    ...
    if (!event.type.startsWith('notebook/')) return
